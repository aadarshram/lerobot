...
class ACTPolicyWithAttention:
    ...
    def select_action(self, observation: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor, List[np.ndarray]]:
        ...
        if attention_weights_capture:
            ...
            attention_maps, proprio_attention, task_attention = self._map_attention_to_images(attn, image_spatial_shapes) # (Ad): Add task support
            self.last_attention_maps = attention_maps
            self.last_proprio_attention = proprio_attention  # Store for visualization
            self.last_task_attention = task_attention # (Ad)
        else:
            ...
            self.last_task_attention = 0.0 # (Ad)
    ...
    def _map_attention_to_images(self,
        ...
        # (Ad): Support for task instruction token. TODO: For now, there is always task input. Add task as feature in config to enable modularity. See how its done in SmolVLA
        task_token_idx = n_prefix_tokens
        n_prefix_tokens += 1
        ...
        # (Ad): Support for task token
        task_attention = 0.0
        if task_token_idx is not None:
            # Extract attention to task token
            if self.specific_decoder_token_index is not None:
                if 0 <= self.specific_decoder_token_index < attention.shape[1]:
                    task_attention_tensor = attention[:, self.specific_decoder_token_index, task_token_idx]
                else:
                    task_attention_tensor = attention[:, :, task_token_idx].mean(dim=1)
            else:
                task_attention_tensor = attention[:, :, task_token_idx].mean(dim=1)

            # Take first batch element
            task_attention = task_attention_tensor[0].cpu().numpy().item()
        ...
        if task_attention is not None:
            if task_attention < global_min:
                global_min = task_attention
            if task_attention > global_max:
                global_max = task_attention
            found_any_valid_map = True
        ...
        # (Ad): Add task support
        if global_max > global_min:
            normalized_task_attention = (task_attention - global_min) / (global_max - global_min)
        else:
            normalized_task_attention = 0.0
        ...
        return final_normalized_attention_maps, normalized_proprio_attention, normalized_task_attention
    ...
    def visualize_attention(self, 
        ...
        # (Ad): Task support
        task_attention = getattr(self, 'last_task_attention', 0.0)  
        ...
        for i, (img, attn_map) in enumerate(zip(images, attention_maps)):
            ...
            # (Ad): Add task support
            if task_attention > 0:
                colour_intensity = int(255 * task_attention)
                task_color = (0, colour_intensity, colour_intensity)  # Cyan
                if use_rgb:
                    task_color = (0, colour_intensity, colour_intensity)

                # Rectangle dimensions
                rect_height = 40
                rect_y_start = h - rect_height

                # Draw filled rectangle at the bottom of the image
                cv2.rectangle(vis, 
                            (0, rect_y_start), 
                            (w, h), 
                            color=task_color, 
                ...
                text = f"Task Attention: {task_attention:.3f}"
                font_scale = 0.7
                font_thickness = 2
                font = cv2.FONT_HERSHEY_SIMPLEX
                ...
                text_y = rect_y_start + (rect_height + text_height) // 2

                cv2.putText(vis, text, (text_x, text_y), font, font_scale, (0, 0, 0), font_thickness)
