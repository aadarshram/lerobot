...
class ACTLangPolicy(PreTrainedPolicy):
    ...
    def select_action(self, batch: dict[str, Tensor], force_model_run: bool = False) -> Tensor:
        ...
        # (Ad): Refactor multi-camera observation acc. 
        ...
        elif force_model_run: # (Ad): Throw away chunks to get per-step model inference: Smoother attention visualization
...
class ACT(nn.Module):
    ...
    def __init__(self, config: ACTLangConfig):
        ...
        if self.config.use_vae:
            ...
            num_input_token_encoder += 1 # (Ad): task token
            if self.config.robot_state_feature:
        ...
        n_1d_tokens += 1 # (Ad): for task token
        if self.config.robot_state_feature:
            n_1d_tokens += 1
        if self.config.env_state_feature:
            n_1d_tokens += 1
        ...
        # (Ad): Fixed one hot embeddings for tasks
        task_embed = self.create_task_embeddings()
        self.register_buffer('task_embeddings', task_embed)

        self._reset_parameters()

    # (Ad): Creates a one hot style embedding for the 9 grid positions in TicTacToe
   ...
   # (Ad): Adding the prepare_language function from smolVLA policy
    def prepare_language(self, batch) -> Tensor:
        ...
        grid_positions = []
        for task in tasks:
            grid_pos = task.lower().strip()[-1] # Expecting task to be of pattern  "<some text> <grid_number>"
            ...
            grid_positions.append(int(grid_pos)-1)

        grid_indices = torch.tensor(grid_positions, device=device) # (B,)

        lang_token = self.task_embeddings[grid_indices] # (B, D)
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        if self.config.use_vae and "action" in batch:
            ...
            cls_joint_is_pad = torch.full(
                # (Ad): changed from 2 else 1 to 3 else 2 to account for task token
                (batch_size, 3 if self.config.robot_state_feature else 2), 
                False,
...
class ACTEncoder(nn.Module):
    ...
    def __init__(self, config: ACTLangConfig, is_vae_encoder: bool = False):
        super().__init__()
